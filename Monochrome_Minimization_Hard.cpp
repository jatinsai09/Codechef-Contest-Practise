...
ll calc(ll xmin, ll xmax, ll ymin, ll ymax) {
    if (xmin > xmax || ymin > ymax) {
        return 0;
    }
    return max((xmax - xmin) * ymax, (ymax - ymin) * xmax);
}
    ...
    while (tc--) {
        ...
        for (int i = 1; i < n; i++) {
            for (int j = 1; j < m; j++) {
                ...
                ll red1 = calc(a[0], a[i - 1], b[0], b[j - 1]);
                ll blue1 = calc(a[i], a[n - 1], b[j], b[m - 1]);
                res = min(res, max(red1, blue1));

                ll red2 = calc(a[0], a[i - 1], b[j], b[m - 1]);
                ll blue2 = calc(a[i], a[n - 1], b[0], b[j - 1]);
                res = min(res, max(red2, blue2));
            }
        }
